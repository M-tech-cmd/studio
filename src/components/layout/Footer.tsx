...
import { Phone, Mail, MapPin, Facebook, Twitter, Youtube, Instagram, Linkedin, Clock } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="space-y-4">
            ...
            <div className="flex space-x-4 pt-2">
              {Array.isArray(settings?.socialLinks) && settings.socialLinks.map((link) => {
                const Icon = socialIcons[link.platform];
                return Icon ? (
                  <Link key={link.platform} href={link.url} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors">
